# The S2 Driver is to output the spectrum of the events over the whole
# to some degree, this is a slow control use
...
class L2SpectrumDriver:
    ...
    FRAME_BYTES = HEAD_BYTES + DATA_BYTES   # 5880
    ...
    def _load_base(self):
        ...
        if u16_size % self.FRAME_U16 != 0:
            raise RuntimeError("File length is not an integer number of L2Spectrum frames")
    ...
    def get_small_spec(self,i):
        return (self.get_energy()[:1280],self.get_spec()[:1280])
    ...
    def read_first_time(self):
        ...
        with open(self.fname, "rb") as f:
            ...
            if len(head40) < self.HEAD_BYTES:
                raise RuntimeError("file too short to contain first L2Spectrum frame header")
    ...
    def read_last_time(self):
        ...
        if byte_size < self.FRAME_BYTES:
            raise RuntimeError("file too short to contain one complete L2Spectrum frame")

        nframes = byte_size // self.FRAME_BYTES
        if nframes == 0:
            raise RuntimeError("no complete L2Spectrum frame found")
        ...
        with open(self.fname, "rb") as f:
            ...
            head40 = f.read(self.HEAD_BYTES)
            if len(head40) < self.HEAD_BYTES:
                raise RuntimeError("failed to read last L2Spectrum frame header")
