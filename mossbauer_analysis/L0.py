...
class L0Driver:
    ...
    TAIL_BYTES = 4620   #
    FRAME_BYTES = HEAD_BYTES + DATA_BYTES + TAIL_BYTES   # 274996
    FRAME_U16 = FRAME_BYTES // 2            # 135188
    ...
    def _load_base(self):
        ...
        if u16_size % self.FRAME_U16 != 0:
            raise RuntimeError("File length is not an integer number of L0 frames")
    ...
    def read_first_time(self):
        ...
        with open(self.fname, "rb") as f:
            ...
            if len(head40) < self.HEAD_BYTES:
                raise RuntimeError("file too short to contain first L0 frame header")
    ...
    def read_last_time(self):
        ...
        if byte_size < self.FRAME_BYTES:
            raise RuntimeError("file too short to contain one complete L0 frame")

        nframes = byte_size // self.FRAME_BYTES
        if nframes == 0:
            raise RuntimeError("no complete L0 frame found")
        ...
        with open(self.fname, "rb") as f:
            ...
            head40 = f.read(self.HEAD_BYTES)
            if len(head40) < self.HEAD_BYTES:
                raise RuntimeError("failed to read last L0 frame header")
